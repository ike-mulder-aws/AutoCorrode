...
def _movers_table(movers):
    """Absolute-first movers table (Proposal C). ✎ marks PR-changed theories;
    Δ is machine-adjusted seconds; % is context."""
    lines = ["| | Theory | main | PR | **Δ** | % |",
             "|:--|---|--:|--:|--:|--:|"]
    for t in movers:
        ...
        lines.append(f"| {mark} | {name} | {base} | {cur} | "
...
def render_movers(diff, current, opts):
    ...
    d = diff
    total_delta = d["cur_total_s"] - d["base_total_s"]
    movers = d["movers"]

    # The signal line: changed theories that slowed. This is what a human acts
    # on — and what a future gate would fire on.
    changed_slow = [t for t in movers
                    if t["changed"] and (t["adj_delta_s"] or 0) > 0]

    out = [MARKER,
           f"## ⏱️ Proof timing — {total_delta:+.0f}s total "
           f"({d['cur_total_s']:.0f}s vs `main` {d['base_total_s']:.0f}s)"]

    if not movers:
        out += ["",
                f"No theory moved by ≥{opts.view_floor_s:.0f}s (runner-adjusted) "
                f"— nothing notable. Runner-speed factor ×"
                f"{d['machine_factor']:.2f}.",
                "", _artifacts_line(opts)]
        return "\n".join(out)

    changed_hint = (" **✎ = source changed by this PR** (more likely a real "
                    "effect; unmarked movers on shared runners are often noise)."
                    if d["changed_files_known"] else "")
    out += ["",
            f"Largest absolute changes, slowest first.{changed_hint} Δ is "
            f"runner-speed-adjusted seconds (this run's runner ≈ ×"
            f"{d['machine_factor']:.2f} vs the baseline runner — see *How Δ is "
            f"computed* below); % is context only.",
            "",
            _movers_table(movers)]

    if d["changed_files_known"]:
        if changed_slow:
            bits = ", ".join(f"`{safe(t['theory'])}` {t['adj_delta_s']:+.1f}s "
                             f"({_pct(t['norm_ratio'])})" for t in changed_slow[:3])
            more = (f" (+{len(changed_slow)-3} more)" if len(changed_slow) > 3
                    else "")
            out += ["",
                    f"**👉 {len(changed_slow)} changed theor"
                    f"{'y' if len(changed_slow) == 1 else 'ies'} slowed: "
                    f"{bits}{more}.** Other movers have no source change in this "
                    f"PR and may be runner noise."]
        else:
            out += ["",
                    "**👉 No theory whose source this PR changed slowed past the "
                    "floor.** The movers below are in unchanged theories — likely "
                    "runner noise."]
