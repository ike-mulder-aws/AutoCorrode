...
The movers table has a **`PR (adj)`** column = `PR − (m−1)·main` (the PR time
re-expressed at the baseline runner's speed), so `Δ = PR(adj) − main` is
eye-verifiable. The header total is likewise adjusted (`raw − (m−1)·base_total`)
so it agrees with the Δ column instead of showing the raw before/after gap.

## Comment variants

- **Movers (normal):** header `±Ns adjusted (raw ±Ns, runner ×m)`, then the
  absolute-first movers table (✎ = PR-changed), a 👉 line naming changed
  theories that slowed, and a collapsed slowest-commands hotspot list. If
  nothing clears the floor, it says so in one line.
- **Unreliable (large runner gap):** when `|m−1| > --broad-notice` (0.15) the
  header flips to `⚠️ runner ×m … comparison unreliable` and reconciles raw vs
  adjusted totals in one line — the numbers are rough pointers, so lean on ✎.
